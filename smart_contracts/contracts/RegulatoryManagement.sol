...
contract RegulatoryManagement is Ownable, ReentrancyGuard, Pausable {
    ...
    /**
     * @dev Register a user by admin with wallet ID, SSI identifier and user type
    ...
    function registerUserByAdmin(
        ...
        string memory ssiIdentifier,
        UserType userType
    ) external onlyOwner nonReentrant whenNotPaused {
        require(walletAddress != address(0), "Invalid wallet address");
        require(
            bytes(ssiIdentifier).length > 0,
            "SSI identifier cannot be empty"
        );
        require(!isUserRegistered[walletAddress], "User already registered");
        require(
            !ssiIdentifierUsed[ssiIdentifier],
            "SSI identifier already used"
        );
        ...
        // Store user profile
        userProfiles[walletAddress] = newProfile;
        isUserRegistered[walletAddress] = true;
        ssiIdentifierUsed[ssiIdentifier] = true;
        allUsers.push(walletAddress);

        emit UserRegisteredByAdmin(
            walletAddress,
            ssiIdentifier,
            userType,
            msg.sender,
    ...
    function verifyUser(address userAddress) external onlyOwner whenNotPaused {
        ...
        require(
            !userProfiles[userAddress].isSuspended,
            "Cannot verify suspended user"
        );
        ...
        emit PermissionsUpdated(
            userAddress,
            true,
            userProfiles[userAddress].canCreateTokens
        );
    }
    ...
    function unsuspendUser(
        ...
        emit PermissionsUpdated(
            userAddress,
            userProfiles[userAddress].canTrade,
            userProfiles[userAddress].canCreateTokens
        );
    }
    ...
    function getVerifiedUsers() external view returns (address[] memory) {
        ...
        for (uint256 i = 0; i < allUsers.length; i++) {
            if (
                userProfiles[allUsers[i]].isVerified &&
                !userProfiles[allUsers[i]].isSuspended
            ) {
                count++;
            }
        }
