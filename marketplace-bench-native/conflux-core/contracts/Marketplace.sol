...
///
/// Ported verbatim from ../../../marketplace-bench/contracts/Marketplace.sol for the
/// Conflux Core Space (native CVM) leg — no source changes were required. `payable`/
/// `msg.value`/`.call{value:}` all behave identically on CVM; CFX is the native coin in
/// the same 18-decimal "Drip" unit family ETH uses, so the payment logic needed no
/// adjustment. See ../README.md for CVM-specific adjustments that *were* needed elsewhere
/// (deployment/signing, not contract source).
